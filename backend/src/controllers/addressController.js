...
// =====================================================
// ADMIN FUNCTIONS - Manage addresses for any user
// =====================================================

// @desc    Get addresses for a specific user (Admin)
...
// @desc    Create address for a specific user (Admin)
// @route   POST /api/addresses/user/:userId
// @access  Private (Admin)
exports.createAddressForUser = async (req, res) => {
  try {
    ...
    console.error('Create address for user error:', error);
    res.status(500).json({
      success: false,
      message: 'Error creating address',
      error: error.message
...
// @desc    Update address for a specific user (Admin)
// @route   PUT /api/addresses/user/:userId/:addressId
// @access  Private (Admin)
  ...
  try {
    ...
    await query(
      `UPDATE user_addresses SET 
        ...
        updated_at = CURRENT_TIMESTAMP
       WHERE id = ? AND user_id = ?`,
      [address_label, recipient_name, phone, address, city, province, postal_code, country, is_default ? 1 : 0, addressId, userId]
    );
    ...
    console.error('Update address for user error:', error);
    res.status(500).json({
      success: false,
      message: 'Error updating address',
      error: error.message
...
// @desc    Delete address for a specific user (Admin)
// @route   DELETE /api/addresses/user/:userId/:addressId
// @access  Private (Admin)
exports.deleteAddressForUser = async (req, res) => {
  try {
    ...
    console.error('Delete address for user error:', error);
    res.status(500).json({
      success: false,
      message: 'Error deleting address',
      error: error.message
...
// @desc    Set default address for a specific user (Admin)
// @route   PUT /api/addresses/user/:userId/:addressId/default
// @access  Private (Admin)
exports.setDefaultAddressForUser = async (req, res) => {
  try {
    const { userId, addressId } = req.params;
    ...
    console.error('Set default address for user error:', error);
    res.status(500).json({
      success: false,
      message: 'Error setting default address',
      error: error.message
